...
def van_der_pol_solve(N, N_Traj):
    ...
    u1 = 2.0 * np.random.rand(1, N_Traj) - 1.0
    for i in range(0, N):
    ...
    for i in range (0, N):
        #nowu1 = np.array(u0[i, :]).reshape((1, N_Traj))
        #nowu2 = np.array(u1[i, :]).reshape((1, N_Traj))
        #nowu = np.concatenate([nowu1, nowu2], axis=0)
        nowu = np.array(u0[i, :])
...
def nonlinear_forApproach3_solve(N, N_Traj):
    ...
    f = lambda t, x, u: np.array([-0.1 * x[0, :] + u[0, :], -1 * x[1, :] + 1 * x[0, :] ** 4 - 2 * x[0, :] ** 2 + u[1, :]])
    k1 = lambda t, x, u: np.array(f(t, x, u))
    k2 = lambda t, x, u: np.array(f(t + h / 2.0, x + 0.5 * h * k1(t, x, u), u))
    k3 = lambda t, x, u: np.array(f(t + h / 2.0, x + 0.5 * h * k2(t, x, u), u))
    k4 = lambda t, x, u: np.array(f(t + h, x + h * k3(t, x, u), u))
    ...
    uConst = 2.0 * np.random.rand(1, N_Traj) - 1.0
    for i in range(0, N):
        u0 = np.concatenate([u0, uConst], axis=0)
...
for eps in range(0, epoch):
    ...
    for i in range(0, batch):
        ...
        #PHIX = torch.cat([inputs_x, PHIX], dim=0)
        #PHIY = torch.cat([inputs_y, PHIY], dim=0)

        PHIY_U = torch.cat([PHIX, inputs_u], dim=0)
        K_hat = PHIY @ torch.pinverse(PHIY_U)
        A_hat = K_hat[0: Nlift, 0: Nlift]
        B_hat = K_hat[0: Nlift, Nlift : Nlift + m]
        B_hat = torch.reshape(B_hat, (len(B), m))
        A = A_hat
        B = B_hat
        #A = A_hat
        #B = B_hat
        A_temp = A.detach().numpy()
        A_temp = np.array(A_temp)
        B_temp = B.detach().numpy()
        B_temp = np.array(B_temp)
        K_temp = K_hat.detach().numpy()
        K_temp = np.array(K_temp)
        PHIX_temp = PHIX.detach().numpy()
        PHIX_temp = np.array(PHIX_temp)
        PHIY_temp = PHIY.detach().numpy()
        PHIY_temp = np.array(PHIY_temp)
        ...
        print("\nLoss", Loss)
        if only_rec == 1:
            print("\n only reconstrunction")
        optimizer.zero_grad()
        Loss.backward()
        optimizer.step()
        print("\nWeight updated!")
        print("{:d} batch".format(i))
        print("{:d} epoch".format(eps))
        print("\nA:", A)
        print("\nB:", B)
        torch.save(net, 'AutoEncoder_20220418_duffing_2.pkl')
        scio.savemat('DeepLearning_KoopmanControl_Approach3_duffing_2.mat',  {'A': A_temp, 'B': B_temp, 'K': K_temp, 'PHIX': PHIX_temp, 'PHIY': PHIY_temp, 'X': X, 'Y': Y, 'U': U})
        if Loss < 0.0001 and Loss_lin < 0.001 and Loss_pred < 0.001:
            break
    if Loss < 0.0001 and Loss_lin < 0.001 and Loss_pred < 0.001:
        break
