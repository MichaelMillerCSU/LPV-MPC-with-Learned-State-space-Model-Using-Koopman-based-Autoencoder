...
def costFunction(uSequence, r , AB, C, x0, pastu, Np, Nc, d):
   ...
   # logY = np.array(logY)
    ...
    x = x0.transpose()
    cost = 1000 * np.sum(np.square(logY)) + 1 * np.sum(np.square(uSequence)) + 0.1 * np.sum(np.square(delta_u)+uSequence[0] - pastu)
    return cost
#
...
for i in range(0,maxStep):
    ...
    A_error.append(np.linalg.norm((A - A_prev), ord=2) /  np.linalg.norm(A, ord = 2))
    B_error.append(np.linalg.norm(B - B_prev, ord=2) /  np.linalg.norm(B, ord = 2))
    A = A_prev
    B = B_prev
