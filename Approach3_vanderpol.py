...
for i in range(0,maxStep):
    ...
    if i < maxStep / 2.0:
        ...
        r1 = [np.array(1 * np.power(-1, math.ceil(i / 200))) for j in range(i, i + MPCHorizon)]
    # r1 = [0.7 * np.array([[np.sin(j / (20 + 0.01 * j))]])  for j in range(i, i + MPCHorizon)]
    ...
    # else :
    #     K_A = ((i + 1) * K_A + q * ylift @ xlift_u.transpose()) / ((i + 1) + q)
